...
export async function resolveOwnedOpenClaim(opts: {
  ...
  if (opts.hintClaimId && isObjectId(opts.hintClaimId)) {
    ...
    if (hinted) {
      ...
      if (recipientOwned || donorOwned) return hinted;
    }
  }
...
export async function executePendingAction(opts: {
  ...
  if (opts.pending.kind === "LATE") {
    return executeLate({ userId: opts.userId, role: opts.role, claim, delayMinutes: opts.pending.delayMinutes });
  }
...
async function executeLate(opts: {
  ...
  const minutes = Math.min(180, Math.max(1, Math.round(opts.delayMinutes ?? 10)));
